...
from functions.ZP_commons.planning import check_self_conflict_Simple, assign_helper_path
from functions.ZP_commons.existing_paths import invalidate_paths_onwards
from functions.SequentialAStar_G_better import group_2_away_agents, AccommodateHMove_NoPartitions
...
def SAS_1(token, dijkstras_map):
    ...
    while there_is_self_conflict:
        ...
        for agent in token.agents:        # For all free agents (They request the token.)
            ...
            if token.agents[planner_agent_id-1].path ==[]:  # Agent requires path planning.
                ...
                if (token.agents[planner_agent_id-1].state == 0 and agent_type=="Home" and token.unassigned_home_tasks) or (token.agents[planner_agent_id-1].state == 0 and agent_type=="Guest" and token.unassigned_guest_tasks) or token.agents[planner_agent_id-1].state != 0:
                    ...
                    try:
                        ...
                        if (token.agents[planner_agent_id-1].state == 0 and agent_type=="Home" and token.unassigned_home_tasks):
                            best_task = closest_task(dijkstras_map, token.agents[planner_agent_id-1].location, token.unassigned_home_tasks)  # Find the Best Task to Assign
                            ...
                            token.unassigned_home_tasks.remove(best_task)                                # Remove the task from unassigned_tasks
                        ...
                        elif (token.agents[planner_agent_id-1].state == 0 and agent_type=="Guest" and token.unassigned_guest_tasks):
                            best_task = closest_task(dijkstras_map, token.agents[planner_agent_id-1].location, token.unassigned_guest_tasks)  # Find the Best Task to Assign
                            token.agents[planner_agent_id-1].assigned_task = best_task              # Assign the best task to the agent
                            token.agents[planner_agent_id-1].task_assignment_time = token.time
                            token.unassigned_guest_tasks.remove(best_task)                                # Remove the task from unassigned_tasks
                    ...
                    NoTaskFreeAgents.append(token.agents[planner_agent_id-1])
        ...
        NoTaskFreeAgentGroups=group_2_away_agents(NoTaskFreeAgents,token.time, dijkstras_map)
        #for idx, group in enumerate(G_groups):
        #    print(f"Group {idx + 1}: {group}")

        for NoTaskFreeAgentGroup in NoTaskFreeAgentGroups:

            try:
                all_intraloop_helper_ids_paths_pathstates_accommodation = AccommodateHMove_NoPartitions(current_time+1, dijkstras_map, token, NoTaskFreeAgentGroup) #Calculate a set of paths once.
                if all_intraloop_helper_ids_paths_pathstates_accommodation:
                    for helper_agent_id, helper_agent_path, helper_agent_path_states in all_intraloop_helper_ids_paths_pathstates_accommodation:
                        if len(helper_agent_path) > 1:
                            ...
                            token.agents[helper_agent_id-1].path.append((token.agents[helper_agent_id-1].location[0], token.agents[helper_agent_id-1].location[1], current_time+1))
                            token.agents[helper_agent_id-1].path_states.append(PathState(path_state="Idle - No Task Available", agent_role="Planner", time=current_time + 1, related_agent_ids=None))
                            token = update_occupied_spacetime_Simple(token, [(token.agents[helper_agent_id-1].location[0], token.agents[helper_agent_id-1].location[1], current_time), (token.agents[helper_agent_id-1].location[0], token.agents[helper_agent_id-1].location[1], current_time+1)])
        ...
        if there_is_self_conflict:
            token = copy.deepcopy(token_great_backup)

            for agent in token.agents:
                if agent.path_set == 0:
                    agent_id = agent.id
                    token = invalidate_paths_onwards(agent_id, token, current_time)
    ...
    token.home_replan_counter += h_replan_count
    token.guest_replan_counter += g_replan_count
