    ...
    while problem_found==True:
        ...
        if problem_found==True:
            ...
            if helper_path_found == True:
                ...
                # Calculate a new tentative path to the goal. (Starting from the end of fixed_planner_path)
...
def ZPG1(token, dijkstras_map, partition_map, border_map, gates, guest_map):
    ...
    for i in range(len(token.agents)):
        ...
        if agent.path_states and agent.path_states[0]:
            ...
            if (next_state == "Planner - Gated to Other Partition" or next_state == "Helper - Making One Move Back") and agent.gating_path_set == 0:
                ...
                first_gate = next_path_state.gate_or_border_before
                second_gate = next_path_state.gate_or_border_after
                gating_pairs_id = next_path_state.related_agent_ids[0]

                first_minimum_effective_distance, first_path_of_danger = calculate_minimum_effective_distance(first_gate, home_agents_and_nextmoves, token.agents, partition_map[first_gate[1]][first_gate[0]], dijkstras_map)
                second_minimum_effective_distance, second_path_of_danger = calculate_minimum_effective_distance(second_gate, home_agents_and_nextmoves, token.agents, partition_map[second_gate[1]][second_gate[0]], dijkstras_map)

                #print(f"Effective Distances for Agent {agent.id} is {first_minimum_effective_distance} and {second_minimum_effective_distance}")
                # First and second eff dist will be different Bcs #G in the gate's partition are taken into account and even in ZPG1 the agent at the other gate won't be taken into account bcs it is not in the same partition.
                if first_minimum_effective_distance > 3 and second_minimum_effective_distance > 3:
                    token.agents[agent.id-1].gating_path_set = 1
                    ...
                    token = invalidate_paths_onwards(agent.id, token, current_time)
    ...
    while there_is_self_conflict:
            ...
            if token.agents[planner_agent_id-1].path ==[]:  # Agent requires path planning. (Since it doesn't have a path, makes sense.)
                ...
                if ((token.agents[planner_agent_id-1].state == 0 and token.unassigned_tasks) or token.agents[planner_agent_id-1].state != 0):      # Free Agent : Needs a Task and a Path or Replan Agent: Just needs a new path.
                    ...
                    try:
                        ...
                        token = update_occupied_spacetime_w_gating_protection(token, [(token.agents[planner_agent_id-1].location[0], token.agents[planner_agent_id-1].location[1], current_time), (token.agents[planner_agent_id-1].location[0], token.agents[planner_agent_id-1].location[1], current_time+1)], [token.agents[planner_agent_id-1].current_path_state] + token.agents[planner_agent_id-1].path_states ,partition_map, gates, guest_map)
                    ...
                    token.agents[planner_agent_id-1].path.append((token.agents[planner_agent_id-1].location[0], token.agents[planner_agent_id-1].location[1], current_time+1))
                    token.agents[planner_agent_id-1].path_states.append(PathState(path_state="Idle - No Task Availabe", agent_role="Planner", time=current_time + 1, related_agent_ids=None))
                    token = update_occupied_spacetime_w_gating_protection(token, [(token.agents[planner_agent_id-1].location[0], token.agents[planner_agent_id-1].location[1], current_time), (token.agents[planner_agent_id-1].location[0], token.agents[planner_agent_id-1].location[1], current_time+1)], [token.agents[planner_agent_id-1].current_path_state] + token.agents[planner_agent_id-1].path_states, partition_map, gates, guest_map)
            ...
            if agent.path_states and agent.path_states[0]:
                ...
                if (next_state == "Planner - Gated to Other Partition" or next_state == "Helper - Making One Move Back") and agent.gating_path_set == 0:
                    ...
                    if first_minimum_effective_distance > 3 and second_minimum_effective_distance > 3:
                        ...
                        # Invalidate paths if effective distance conditions are not met
                        there_is_self_conflict=True
                        complete_replan_required = True
                        #print("X Gating was planned while agents were at the gates and it is not safe.")
                        token = copy.deepcopy(token_great_backup)

                        if agent.path_set ==0:

                            self_conflicting_agent_id = agent.id
                            ...
                            #print(f"Agent {token.agents[self_conflicting_agent_id-1].id} will idle with path {token.agents[self_conflicting_agent_id-1].path} to abort gating.")


                            token.agents[gating_pairs_id-1].path=[(token.agents[gating_pairs_id-1].location[0], token.agents[gating_pairs_id-1].location[1], current_time+1)]
                            token.agents[gating_pairs_id-1].path_states=[PathState(path_state="Idle - No Valid Plan Possible", agent_role="Planner", time=current_time + 1, related_agent_ids=None)]
                            token.agents[gating_pairs_id-1].path_set=1
                            #print(f"Agent {token.agents[gating_pairs_id-1].id} will idle with path {token.agents[gating_pairs_id-1].path} to abort gating.")
        ...
        if self_conflicting_agents:
            ...
            if there_is_self_conflict == False:
