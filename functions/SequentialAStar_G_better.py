...
from functions.ZP_commons.a_star_algorithms import a_star_any_generator_NoPartitions
...
def group_2_away_agents(agents,current_time,dijkstras_map):
    ...
    for agent_index, group_id in agent_groups.items():
        ...
        grouped_agents.setdefault(group_id, []).append((agent.id, (agent.location[0], agent.location[1], current_time)))
...
def SAS_G(token, dijkstras_map):
    ...
    G_groups=group_2_away_agents(token.agents,token.time,dijkstras_map)
