...
from functions.ZP_commons.a_star_algorithms import a_star_algorithm_pickup_or_delivery, a_star_any
from functions.ZP_commons.occupied_spacetime import update_occupied_spacetime_Simple
from functions.ZP_commons.planning import check_self_conflict_Simple, find_closest_unclaimed_parking_spot_to_agent
            ...
            if token.agents[planner_agent_id-1].path ==[]:  # Agent requires path planning.
                ...
                if ((token.agents[planner_agent_id-1].state == 0 and token.unassigned_tasks) or token.agents[planner_agent_id-1].state != 0):
                    ...
                    try:
                        ...
                        path_to_park, path_states_to_park = PathPlannerAStar(dijkstras_map, agent, (token.agents[planner_agent_id-1].location[0], token.agents[planner_agent_id-1].location[1], token.time), (token.agents[planner_agent_id-1].claimed_parking_spot[0], token.agents[planner_agent_id-1].claimed_parking_spot[1]), token)
                        ...
                        # Assign Path to Planner Agent
                        token = update_occupied_spacetime_Simple(token, path_to_park)
